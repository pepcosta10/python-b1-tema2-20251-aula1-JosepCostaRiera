...
def sum_even_numbers_in_list_while(list_numbers):
    suma = 0
    i = 0
    while i < len(list_numbers):          # mentre quedin elements
        if list_numbers[i] % 2 == 0:      # si és parell
            suma += list_numbers[i]
        i += 1                            # avancem l'índex manualment
    return suma


def sum_even_numbers_in_list_for(list_numbers):
    suma = 0
    for num in list_numbers:              # recorre cada element directament
        if num % 2 == 0:                  # si és parell
            suma += num
    return suma
...
def sum_even_numbers_in_list_do_while(list_numbers):
    suma = 0
    if len(list_numbers) == 0:            # protecció per llista buida
        return suma
    i = 0
    while True:                           # simulem el do-while
        if list_numbers[i] % 2 == 0:      # si és parell
            suma += list_numbers[i]
        i += 1
        if i >= len(list_numbers):        # condició de sortida AL FINAL
            break
    return suma
